...
**Intro "hello screensaver"** (`.intro` no HTML, bloco `.intro*` em `main.css`, IIFE no topo do `script.js`): saudação em 10 idiomas antes do conteúdo, terminando em "Hello" e na bandeirinha. Total ≈ 7.15s (era ~17s com 20 idiomas e hold de 320ms/palavra — cortado a pedido, ver conta abaixo). Decisões que parecem estranhas e são de propósito:
...
- Durações em `tokens/motion.css` (`--intro-*`), lidas de volta pelo `script.js` via `getComputedStyle` — mesmo padrão do `--panel-w`. Mexa nos tokens, não nos números do JS. `--intro-hold` (pausa por palavra) e `--intro-mark-hold` (pausa da marca antes de dissolver) são números **separados** de propósito — antes a marca reaproveitava `--intro-hold-last` (a pausa do "Hello"), acoplamento sem razão semântica que foi corrigido junto com o corte de 20→10 idiomas.
- A conta do total: 10 palavras não-finais a `2×--intro-fade + --intro-hold` (200+200+120 = 520ms) cada = 5.2s, mais "Hello" a `2×--intro-fade + --intro-hold-last` (200+200+1000 = 1.4s), mais a marca a `--intro-reveal + --intro-mark-hold` (400+150 = 550ms). Total ≈ 7.15s. Se precisar aproximar mais de "6-7s", o primeiro número a cortar é `--intro-hold` (120ms), não os hold fixos do "Hello" ou da marca.
- Os idiomas não-latinos (grego não entrou nesta lista; cirílico, árabe, CJK) caem no `system-ui` do stack — Inter não cobre esses scripts, e isso é o comportamento esperado, não um bug de fonte.
