...
2. **Os arquivos andam juntos** em qualquer deploy: `index.html`, `wiki.html`, `changelog.html`, `prompts.html`, `styles/main.css`, os cinco arquivos em `styles/tokens/`, `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs`, `scroll.mjs`, `vendor/cuelume/**`, `vendor/lenis/**`, `data/contributions.json`, `favicon.svg`. Subir o HTML sem o CSS completo (main.css + tokens) derruba o site inteiro, não só o estilo — `main.css` sozinho, sem os tokens, quebra tudo porque cada `var()` fica sem definição.
3. **Antes de qualquer deploy/commit**, valide: HTML bem formado (tags fechando corretamente) e `node --check` sem erro em `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs` e `scroll.mjs`. Rode local antes de subir (`python3 -m http.server` na raiz e abra no browser — abrir o `index.html` via `file://` não serve, os caminhos absolutos `/styles/main.css` e `/script.js` não resolvem).
...
- [ ] `node --check` passa em `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs`, `scroll.mjs`
- [ ] Os arquivos (`index.html`, `wiki.html`, `changelog.html`, `prompts.html`, `styles/main.css`, `styles/tokens/*.css`, `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs`, `scroll.mjs`, `vendor/cuelume/**`, `vendor/lenis/**`, `data/contributions.json`, `favicon.svg`) estão consistentes entre si no commit
