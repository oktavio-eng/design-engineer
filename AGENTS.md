...
- **`--intro-step` é 180ms, medido, não estimado.** O primeiro corte foi feito a 1s por estimativa; depois a página de referência foi medida de verdade (observer no DOM: ~182-183ms entre trocas, constante) e o token desceu pra `--duration-180`. Não use `183ms` literal: a diferença é um quinto de frame a 60Hz — abaixo do que o `setTimeout` entrega e do que o olho vê — e o passo já existia na escala. O efeito é um embaralhado rápido, não um slideshow.
...
- **Os 6 idiomas nascem todos no DOM de uma vez, empilhados numa célula de grid** (`.intro__langs` / `.intro__lang`, criados no JS a partir de `GREETINGS`). A linha então mede a palavra mais larga e nunca mais muda de largura. Isso não é micro-otimização: com o cross-fade antigo existia um frame vazio entre as palavras que escondia o reflow, e o corte não tem frame vazio — sem o empilhamento, o `&bull;` pularia lateralmente a cada troca. A 180ms isso seria tremor, não movimento. Trocar de idioma é só mover a classe `.on` de um span pro outro.
...
- A conta do total: 5 palavras (Hola, Bonjour, Olá, こんにちは, 你好) a `--intro-step` (180ms) cada = 900ms, mais o "Hello" a `--intro-step + --intro-hold-last` (180+640 = 820ms), mais o fade dele a `--intro-fade` (200ms), mais a marca a `--intro-reveal + --intro-mark-hold` (400+150 = 550ms). Total ≈ 2.47s até liberar o conteúdo, mais os 500ms de `--intro-out` (que já rodam em cross-fade com o stagger) até o overlay sair do DOM.
