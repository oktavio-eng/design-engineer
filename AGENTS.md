...
- Com eixo variável, **peso não precisa ser múltiplo de 100**. O experimento de tipografia plana usa 460, que é uma instância interpolada real — o `font-synthesis: none` do `body` continua valendo e recusaria qualquer peso falso.
...
Ligado por um `<link>` no `<head>` do `index.html`, **depois** do `main.css`. Achata a escala tipográfica inteira num tamanho só (1.08rem) e num peso só (460), deixando toda a hierarquia por conta do cinza, do espaço e do ritmo. É uma pergunta de craft, não um estado estável do design system.
...
- **O peso único é 460, não 500.** Só é possível porque Geist entra como variável de verdade (`font-weight: 100 900` no `@font-face`); qualquer número do intervalo é uma instância interpolada. Se a família um dia voltar a ser servida em instâncias estáticas, 460 vira 500 arredondado sem ninguém avisar.
