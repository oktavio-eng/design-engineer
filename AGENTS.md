...
- **[CHANGELOG.md](CHANGELOG.md)** (raiz) — o log técnico interno, um bloco por dia, newest first; o `/changelog` do site é a versão narrada pro público. Toda mudança que vai pra `main` ganha um bullet aqui — no mesmo PR ou num PR "Changelog: …" logo depois (é assim que tem sido feito). **Bullet curto:** o quê, onde e o PR; o porquê vai pro `docs/*.md` do tópico e o bullet aponta. **Só o mês corrente fica na raiz** — quando o mês fecha, o bloco vai verbatim pra `docs/changelog/AAAA-MM.md` com link nos dois sentidos (agosto de 2026 já está lá). Por quê e o que foi rejeitado: [docs/architecture.md](docs/architecture.md), "Documentação". Não estava neste índice até 28/08/2026 e por isso um dia inteiro de trabalho quase ficou de fora.
...
- **[docs/figma-workflow.md](docs/figma-workflow.md)** — desde 29/08/2026 o craft é desenhado no Figma (arquivo `Oktavio`, key `H0X3JmAFN32LWINqemEhXs`) e implementado a partir do frame: o que é fonte de quê, como pedir ("analise" vs "faz", sem PR até pedir, tokens sincronizam antes dos componentes), as decisões já tomadas lá que **não** devem ser "corrigidas" de volta pro que produção faz (Article 652/padding 6/hover só fill sem transição, Doc icon 50×60), e os gotchas de escrever no Figma via MCP.
- **[docs/nova-home-projetos.md](docs/nova-home-projetos.md)** — o protótipo da nova home em `/testes/prototipo-nova-home`: Projects como carrossel de cards e o detalhe do projeto como folha grande com galeria. O que bate com o Figma, o que a IA interpretou, os três eixos ainda em aberto (carrossel sangrando × na coluna, drawer × modal, hover anel × fill) e os campos novos de conteúdo (`summary`, `gallery`).
- **[docs/messages.md](docs/messages.md)** — validação e envio do composer, Web3Forms + arquivo D1, testes e migração do Supabase.
- **[docs/studio.md](docs/studio.md)** — dashboard, API Worker/D1, login local, Cloudflare Access, superfícies do DS, comandos de teste e publicação pendente.
...
2. **Os arquivos andam juntos** em qualquer deploy: `index.html`, `wiki.html`, `changelog.html`, `prompts.html`, `styles/main.css`, os cinco arquivos em `styles/tokens/`, `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `content-sync.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs`, `scroll.mjs`, `vendor/cuelume/**`, `vendor/lenis/**`, `data/contributions.json`, `favicon.svg`. Subir o HTML sem o CSS completo (main.css + tokens) derruba o site inteiro, não só o estilo — `main.css` sozinho, sem os tokens, quebra tudo porque cada `var()` fica sem definição. Depois do cutover do Studio (ver [docs/studio.md](docs/studio.md), passo 6), `content.js`, `portfolio-content.js` e `prompts.mjs` passam a ser servidos pelo Worker e saem do deploy estático via `.vercelignore` — continuam no repositório como seed e para o `dev:admin`.
3. **Antes de qualquer deploy/commit**, valide: HTML bem formado (tags fechando corretamente) e `node --check` sem erro em `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `content-sync.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs` e `scroll.mjs`. Rode local antes de subir (`python3 -m http.server` na raiz e abra no browser — abrir o `index.html` via `file://` não serve, os caminhos absolutos `/styles/main.css` e `/script.js` não resolvem).
...
- [ ] `node --check` passa em `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `content-sync.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs`, `scroll.mjs`
- [ ] Os arquivos (`index.html`, `wiki.html`, `changelog.html`, `prompts.html`, `styles/main.css`, `styles/tokens/*.css`, `script.js`, `chrome.js`, `content.js`, `portfolio-content.js`, `favicons.js`, `intro.js`, `mail.js`, `content-sync.js`, `cmd.mjs`, `prompts.mjs`, `portfolio.mjs`, `contrib.mjs`, `cursor.mjs`, `sound.mjs`, `scroll.mjs`, `vendor/cuelume/**`, `vendor/lenis/**`, `data/contributions.json`, `favicon.svg`) estão consistentes entre si no commit
