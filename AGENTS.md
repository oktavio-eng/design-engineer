...
- Revalidado em Chrome real via CDP em 07/08/2026 (headless novo, `--headless=new`, screenshot real de cada estado — o modo com janela travou em `Page.captureScreenshot` por occlusion, não é uma opção viável aqui): 185/185 elementos de texto visíveis em 17.92px/460, `document.fonts.check("17.92px Geist")` true (fonte de fato renderizada, não só declarada), `getComputedStyle` de um `h1` e de um link de `.row` confirmando 17.92px/460/Geist, sem overflow horizontal em desktop (1280) nem no reflow em 320px (`scrollWidth === clientWidth` nos dois), e os três estados do command palette (vazio, com resultados, detalhe) sem erro de console.
