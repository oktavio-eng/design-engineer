...
**Intro "hello screensaver"** (`.intro` no HTML, bloco `.intro*` em `main.css`, IIFE no topo do `script.js`): saudação em 7 idiomas antes do conteúdo, terminando em "Hello" e na bandeirinha, ordem ocidental → não-latino → "Hello". Total ≈ 5.6s (era ~17s com 20 idiomas e hold de 320ms/palavra, depois ~7.15s com 10 idiomas — cortado a pedido em dois passos, ver conta abaixo). Decisões que parecem estranhas e são de propósito:
...
- A conta do total: 7 palavras não-finais (Ciao, Hola, Bonjour, Hallo, Olá, こんにちは, 你好) a `2×--intro-fade + --intro-hold` (200+200+120 = 520ms) cada = 3.64s, mais "Hello" a `2×--intro-fade + --intro-hold-last` (200+200+1000 = 1.4s), mais a marca a `--intro-reveal + --intro-mark-hold` (400+150 = 550ms). Total ≈ 5.59s até liberar o conteúdo, mais os 500ms de `--intro-out` (que já rodam em cross-fade com o stagger) até o overlay sair do DOM.
