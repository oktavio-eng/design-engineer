...
- `npm run test:visual` — compara as capturas do build (dezenove em 05/09/2026: catorze do site, cinco do Studio — `studio-inbox-*` e `studio-typeface-*`) em `storybook-static/` com `tests/visual/baselines/`; por isso exige `npm run build-storybook` antes. Para uma mudança visual intencional, rode `UPDATE_VISUAL_BASELINES=1 npm run test:visual`, inspecione cada PNG e então rode o comando normal de novo.
...
**Instalador travando na extração (06/09/2026):** rodando de dentro do Claude Code, `npx playwright install` baixa o zip em segundos (`DEBUG=pw:install` mostra `download complete`) e então fica parado em `extracting archive` — horas, sem erro, deixando um `.app` sem `Frameworks/` (foi isso que quebrou o cache em 04/09; o `launch` morre com `dlopen … Framework (no such file)`). A rede não é o problema (`curl` faz 9–28 MB/s na CDN) nem o `unzip` do sistema (3 s): é o extrator em Node do Playwright que não completa nesse ambiente. Contorno que funcionou: baixar os dois zips direto (`https://storage.googleapis.com/chrome-for-testing-public/<versão do Chrome>/mac-arm64/chrome-mac-arm64.zip` e `…/chrome-headless-shell-mac-arm64.zip` — a versão está na primeira linha do log do instalador, `145.0.7632.6` para a revisão 1208), `unzip -q` cada um dentro de `~/Library/Caches/ms-playwright/chromium-1208/` e `chromium_headless_shell-1208/`, e `touch INSTALLATION_COMPLETE` em cada diretório, que é o marcador que o instalador gravaria. `chromium.launch()` sobe os dois normalmente depois disso. E se for matar um `npx playwright install` pendurado, mate o `node …/playwright install` filho, não só o `npx` — o `alarm`/timeout no processo pai não chega ao filho e ele fica órfão segurando o pipe.

**Baselines dark defasadas (06/09/2026):** ao regenerar a matriz inteira, `colors-dark`, `command-search-dark`, `people-selected-dark` e `prompts-empty-dark` voltaram com quase todos os pixels deslocados em ≤ 25 níveis — a rampa dark foi escurecida no código depois que elas nasceram (o swatch antigo mostra `--bg oklch(0.24…)`, o código diz `0.179`) e o deslocamento ficou abaixo do `threshold: 0.12` do comparador, então o CI nunca reclamou. Não entraram no PR #92 (não era a mudança dele); foram refrescadas no PR seguinte, sozinhas e inspecionadas, junto com a folga do `--row-dim`. Regra que fica: quando um token muda abaixo do limiar do comparador, regenere a baseline mesmo assim — o teste não vai cobrar, mas o PNG passa a mentir sobre o que o código pinta.
...
A regressão visual usa apenas Playwright + o comparador já trazido pelo Vitest Browser. A matriz cobre tokens em light/dark, tipografia normal/Flat type, 320px, disclosure expandido e command palette aberto. Todas as capturas rodam com `prefers-reduced-motion: reduce`, esperam fontes, animações e dois frames idênticos; a story de tipografia também usa CDP para provar que Geist, Geist Mono e Geist Pixel foram realmente pintadas como webfonts. A comparação tem duas travas: até 4% no diff cru e até 0,8% depois do mesmo blur de 2px nas duas imagens — os dois são por caso (`blurRadius`, `perceptualLimit`): `prompts-modal-light` roda com blur 4 e 1,2% desde 30/08/2026, porque o bloco de prompt em Geist Mono faz o avanço dos glifos derivar ao longo de cada linha entre CoreText (macOS, onde a baseline nasce) e FreeType (Linux, CI) — medido no PR #87, com um deslocamento sintético de 4px ainda reprovando com folga (2,07% vs 1,2%); os números estão no comentário do caso. O primeiro impede uma divergência grande de passar; o segundo reduz ruído de rasterização dos glifos entre Chrome/macOS e Chromium/Linux e ainda reprova deslocamentos sintéticos de layout a partir de 4px nos casos estreitos e tipográficos. As duas métricas de todos os casos aparecem até no log verde. Em falha, `artifacts/visual/` recebe `actual` e `diff`; o diretório é ignorado pelo Git e publicado pelo CI.
