...
- Mesmo DS do portfólio: fundo externo e sidebar `--bg`, superfície principal `--white`, cards com `--radius-24` (eram 32 até 06/09/2026, PR #92 — o 32 lia grande demais ao lado do site); botões, tags e controles compactos com `--radius-full`. Sem linhas divisórias. Hover, cursor e sons compartilham os componentes existentes. O cursor continua visível sobre inputs e acima dos diálogos nativos. Tooltips em controles de ícone compartilham o visual e os tokens de entrada/saída do gráfico de contribuições, aparecem por hover ou foco e fecham com Escape.

- Editor de conteúdo (06/09/2026, #92/#93): o modal `.admin-editor-dialog` é 100% branco e seus campos usam `--bg` (bege), pra separar campo de superfície sem borda pesada; o diálogo de confirmação menor continua bege. Depois que o `--bg` voltou pra 0.982 o contraste campo × modal ficou baixo, então os campos ganharam stroke de 1px em `--ink-a8` como `box-shadow` inset (não desloca layout) e, no foco, em vez do outline de 2px em `--ink`, um anel tracejado de 1px a 10% (`--ink-a10`) com traço de 8px — `dashed` do CSS não controla o comprimento do traço, por isso é um `rect` SVG com `stroke-dasharray` pintado como `background` sem `viewBox` (px reais do elemento), com versão em `--white` no `data-theme="dark"`. Tokens `--ink-a8`/`--ink-a10` em `colors.css`.
- Busca da caixa de entrada agrupa a digitação com debounce de 150 ms, cancelado ao navegar, trocar filtro ou sair (06/09/2026) — antes cada tecla disparava uma consulta ao D1.
